...
<body>
	...
	<script>
		...
		function selectBookList() {
		    let friendNo = ${friendNo};
		    let nickName = '${sessionScope.loginUser.nickName}';
		    $.ajax({
		        url: '/local/miniPage/book/' + friendNo,
		        success: list => {
		            let data = '';
		            if (list.length === 0) {
		                ...
		                data = list.data.map(book => {
		                    let deleteButton = '';
		                    if (book.nickName === nickName) {
		                        deleteButton = '<a onclick="deletePost(this)">삭제</a>';
		                    }
		...
		function insertPost(){
			...
			if($('textarea').val() === ''){
				...
				$.ajax({
					url : '/local/miniPage/'+friendNo +'/'+postContent+'/'+nickName,
					type: 'post',
					success: confirm =>{
						if(confirm.data === 'Y'){
							$('textarea').val('');
		...
		function deletePost(e){
			if(confirm('삭제하시겠습니까?')){
				...
				$.ajax({
					url: '/local/miniPage/delete/' + postNo,
					type: 'post',
					success: confirm => {
